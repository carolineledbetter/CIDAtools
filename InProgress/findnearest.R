...
mergeClose <- function(x, y, nearby, ...){
  UseMethod('mergeClose')
}

mergeClose.data.frame <- function(x, y,
                                  nearby, nearby.x = nearby, nearby.y = nearby,
                                  by = intersect(names(x), names(y)),
                                  by.x = by, by.y = by,
                                  all = FALSE, all.x = all, all.y = all,
                                  sort = TRUE, suffixes = c(".x",".y"),
                                  no.dups = TRUE,
                                  incomparables = NULL, ...sort = TRUE,
                                  suffixes = c(".x",".y"), no.dups = TRUE,
                                  incomparables = NULL, ...){
  by.x <- setdiff(by.x, nearby.x)
  by.y <- setdiff(by.y, nearby.y)
